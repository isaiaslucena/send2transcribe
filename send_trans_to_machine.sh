...
while [[ ${w} -eq 1 ]] ; do
	...
	if [[ ${arrc} -gt 0 ]] ; then
		...
		for arrn in $(seq 0 ${arr}) ; do
			...
			filemachine=$(jq --raw-output .[$arrn].transc_machine "${queuefile}")
			filesourcemachine=$(jq --raw-output .[$arrn].machine "${queuefile}")
			arrnn=$(( ${arrn} + 1 ))
			...
			if [[ ${tcount} -eq 1 ]] ; then
				firstsourcemachine="${filesourcemachine}"
				firstfname="${filename}"
				firstfmachine="${filemachine}"
			elif [[ ${tcount} -eq 2 ]] ; then
				secondsourcemachine="${filesourcemachine}"
		...
		if [[ ${pidsc} -eq 0 ]] ; then
			...
			"${appdir}/send_trans_to_nosql.sh" "${firstsourcemachine}" "${firstfname}" "${firstfmachine}" &
			# echo "${appdir}/send_trans_to_nosql.sh ${firstsourcemachine} ${firstfname} ${firstfmachine} &"
			# read
			sleep 1
			...
			# add to transcribing
			added=$(jq --compact-output --argjson iadd '{"machine":"'${firstsourcemachine}'", "file":"'${firstfname}'", "transc_machine": "'${firstfmachine}'"}' '. += [$iadd]' "${transcribingfile}")
			echo "${added}" > "${transcribingfile}"


			now=$(date +'%Y-%m-%d %H:%M:%S')
			echo "${now} - ${secondfname} - Starting transcribing file" >> "${logfile}"
			"${appdir}/send_trans_to_nosql.sh" "${secondsourcemachine}" "${secondfname}" "${secondfmachine}" &
			# echo "${appdir}/send_trans_to_nosql.sh ${secondsourcemachine} ${secondfname} ${secondfmachine} &"
			# read
			sleep 1
			...
			# add to transcribing
			added=$(jq --compact-output --argjson iadd '{"machine":"'${secondsourcemachine}'", "file":"'${secondfname}'", "transc_machine":"'${secondfmachine}'"}' '. += [$iadd]' "${transcribingfile}")
			echo "${added}" > "${transcribingfile}"
			...
			echo "${now} - ${firstfname} - Starting transcribing file" >> "${logfile}"
			"${appdir}/send_trans_to_nosql.sh" "${firstsourcemachine}" "${firstfname}" "${firstfmachine}" &
			# echo "${appdir}/send_trans_to_nosql.sh ${firstfname} ${firstfmachine} &"
			# read
			sleep 1
